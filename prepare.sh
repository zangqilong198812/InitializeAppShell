...
showMenu() {
	...
	echo "4. 安装gitflow  （一个简化git flow流程的命令行工具）"
	echo "5. 安装commit hook自动规范代码（需要先安装clang-format并且确保你的根目录有.git文件夹）"
	echo "6. 安装chisel  （fb提供的一个lldb增强工具）"
	echo "7. 更改gem source源到ruby-china （gem自带源在中国太慢，更换到ruby-china，taobao提供的源已停用）"
	echo "8. 安装oh-my-zsh (一个命令行的增强工具，很强大)"
	echo "9. 安装Cocoapods"
	echo "10. 安装carthage"
	echo "================================================================="
	read -p "输入编号：" number
